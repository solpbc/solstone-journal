...
pub fn link_status(ctx: CommandContext<'_>) -> CommandOutput {
    ...
    let (state_str, version_str, persist_note) = if let Some(snap) = live_status {
        ...
        let note = if snap.persist_uncertain {
            "Warning: Sidecar state persistence uncertain. Retry status after disk recovers; do not delete credentials bundle; do not assume prior state rolled back.\n"
        } else {
            ""
        };
        (snap.state, ver, note)
...
mod tests {
    ...
    fn status_reports_sidecar_persist_uncertainty_warning() {
        ...
        assert!(
            output
                .stdout
                .contains("Warning: Sidecar state persistence uncertain.")
        );
        assert_eq!(probe.recorded(), vec![5015]);
    }
