...
#[tokio::test]
async fn enrolled_device_cannot_delete_another_clients_cid() {
    let fixture = Fixture::established(2);
    let ledger_path = fixture.root.join("link/authorized_clients.json");
    let initial_ledger_bytes = fs::read(&ledger_path).expect("ledger bytes");
    let ledger_read = read_authorized_clients(&ledger_path);
    let (sender, receiver) = watch::channel(DeviceDoorAuthorization::from(ledger_read));
    let door_router = authorized_router(fixture.root.clone(), receiver);
    let handle = bind_with_authorization(
        options(&fixture, router(fixture.root.clone()), 0),
        door_router,
    ...
    let entries = match read_authorized_clients(&ledger_path) {
        AuthorizedClientsRead::Present(entries) => entries,
        _ => panic!("ledger not present"),
    };
    let client_1_fingerprint = &entries[1].fingerprint;
    let path = format!("/app/network/api/clients/{client_1_fingerprint}");

    let response = request_once(
        Arc::new(fixture.client_config(0)),
        "127.0.0.1",
        door_port(handle.door_outcome()),
        "DELETE",
        &path,
    ...
    assert_eq!(parsed["reason_code"], "device_removal_forbidden");

    let post_ledger_bytes = fs::read(&ledger_path).expect("post ledger bytes");
    assert_eq!(initial_ledger_bytes, post_ledger_bytes);
