...
fn inconsistent_map_entries_are_rejected() {
    ...
    let id = MarkId::derive(RemovalClass::OwnerRawRelease, &target);
    let invalid = json!({
        "version": 1,
        "marks": {
            "not-the-mark-id": {
                ...
                "proposal": {"files": 2, "bytes": 42, "reason": "owner requested it"},
                "state": "marked"
            }
        }
    });
...
fn reconcile_keeps_a_stale_failed_entry_and_its_staged_path() {
    ...
    .unwrap();
    let id = MarkId::derive(RemovalClass::PolicyRawRelease, &held);
    let after = reconcile(bed.path(), RemovalClass::PolicyRawRelease, &[], "second").unwrap();
...
fn reconcile_leaves_other_classes_unchanged() {
    ...
    .unwrap();
    let owner_id = MarkId::derive(RemovalClass::OwnerRawRelease, &owner);
...
fn record_failure_converts_a_marked_entry_and_creates_a_missing_one() {
    ...
    .unwrap();
    let marked_id = MarkId::derive(RemovalClass::PolicyRawRelease, &marked_target);
    ...
    assert_eq!(created.marks[&missing_id].proposal.files, 0);
...
fn every_reconcile_branch_is_exercised_in_one_fixture() {
    ...
    .unwrap();
    assert!(!after.marks.contains_key(&MarkId::derive(
        RemovalClass::PolicyRawRelease,
        &stale_marked
    )));
    assert!(after.marks.contains_key(&MarkId::derive(
    ...
    let failed_id = MarkId::derive(RemovalClass::PolicyRawRelease, &matching_failed);
    assert_eq!(after.marks[&failed_id].proposal, proposal("new failed"));
    assert!(matches!(
        after.marks[&failed_id].state,
        MarkState::Failed(_)
    ));
    let marked_id = MarkId::derive(RemovalClass::PolicyRawRelease, &matching_marked);
    assert_eq!(after.marks[&marked_id].marked_at, "first");
    let new_id = MarkId::derive(RemovalClass::PolicyRawRelease, &new_target);
