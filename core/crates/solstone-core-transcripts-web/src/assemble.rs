...
pub(crate) async fn api_read(
    ...
    let sources = Sources {
        ...
        agents: query.agents.as_deref() == Some("1"),
    };
    let markdown = if let (Some(start), Some(end)) = (query.start.as_deref(), query.end.as_deref())
    {
        cluster_range(&state.journal_root, &day, start, end, sources)
        ...
        Ok(cluster(&state.journal_root, &day, sources))
    };
    match markdown {
...
fn cluster(root: &Path, day: &str, sources: Sources) -> String {
...
fn cluster_range(
    ...
    let date = NaiveDate::parse_from_str(day, "%Y%m%d").map_err(error_response)?;
    let start =
