...
pub fn assemble_sensor_state(segment_dir: &Path) -> Value {
...
pub fn resolve_predecessor(
    ...
    let (previous_start, previous_duration) = segment_start_and_duration_seconds(predecessor)?;
    let (current_start, _) = segment_start_and_duration_seconds(segment)?;
    let previous_end = (i64::from(previous_start.hour) * 3600
        + i64::from(previous_start.minute) * 60
        + i64::from(previous_start.second)
        + i64::try_from(previous_duration).ok()?)
    .rem_euclid(86_400);
...
pub fn compare_screen(previous: &Value, current: &Value) -> Value {
...
pub fn compare_transcript(previous: &Value, current: &Value) -> Value {
...
pub fn classify(vectors: &Value) -> (String, Vec<String>) {
...
mod tests {
    ...
    use super::{
        classify, compare_screen, compare_transcript, detect_segment_change,
