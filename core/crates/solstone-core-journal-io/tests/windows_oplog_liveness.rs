...
use windows_sys::Win32::Storage::FileSystem::{
    ...
    FILE_SHARE_READ, FILE_SHARE_WRITE, FileDispositionInfo, OPEN_EXISTING, ReadFile,
...
#[test]
fn wrong_volume_hint_is_indeterminate_before_share_classification() {
    let temporary = tempfile::tempdir().expect("temporary directory");
    ...
    let refs_root = std::env::var_os("SOLSTONE_JOURNAL_WIN_REFS_ROOT")
        .expect("native Windows gate supplies a distinct ReFS root");
    let refs = tempfile::Builder::new()
        .prefix("solstone-refs-oplog-volume-hint-")
        .tempdir_in(refs_root)
        .expect("create ReFS volume-hint journal");
...
#[test]
fn child_capture_inherits_append_only_not_reader_or_delete_rights() {
    let temporary = tempfile::tempdir().expect("temporary directory");
    let (writer, _health, published) = create_and_publish(temporary.path());
    let mut command = Command::new(std::env::current_exe().expect("current test executable"));
    command.args([
        "--exact",
        "child_capture_append_only_rights_helper",
        "--ignored",
        "--nocapture",
    ]);
    let capture = writer
        .prepare_child_capture(command)
        .expect("prepare append-only child capture");
    let mut child = capture.spawn().expect("spawn append-only rights helper");
    drop(writer);
    assert!(child.wait().expect("wait rights helper").success());
    let bytes = std::fs::read(published).expect("read captured helper output");
    assert!(
        bytes
            .windows(b"append-only-rights-ok".len())
            .any(|window| window == b"append-only-rights-ok")
    );
}

#[test]
#[ignore = "invoked as a child by the append-only capture test"]
fn child_capture_append_only_rights_helper() {
    let mut stdout = std::io::stdout();
    let stdout_handle = stdout.as_raw_handle();
    // SAFETY: the test process owns its standard-output handle for its lifetime;
    // buffers and output pointers below remain valid for each synchronous call.
    #[allow(unsafe_code)]
    unsafe {
        assert_ne!(stdout_handle, INVALID_HANDLE_VALUE);
        ...
        assert_eq!(
            ...
            0,
            "captured stdout must not carry read-data access"
        );

        let process = GetCurrentProcess();
        let mut delete_duplicate = INVALID_HANDLE_VALUE;
        let duplicated = DuplicateHandle(
            process,
            stdout_handle,
            process,
            &mut delete_duplicate,
            DELETE,
            0,
            0,
        );
        if duplicated != 0 {
            CloseHandle(delete_duplicate);
        }
        assert_eq!(
            duplicated, 0,
            "captured stdout must not carry delete access"
        );
    }
    writeln!(stdout, "append-only-rights-ok").expect("write rights marker");
    stdout.flush().expect("flush rights marker");
}
