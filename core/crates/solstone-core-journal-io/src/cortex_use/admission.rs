...
use windows_sys::Win32::Foundation::{
    ERROR_FILE_EXISTS, ERROR_FILE_NOT_FOUND, ERROR_PATH_NOT_FOUND, GENERIC_WRITE,
};
#[cfg(windows)]
use windows_sys::Win32::Storage::FileSystem::{
    FILE_READ_ATTRIBUTES, FlushFileBuffers, MoveFileExW, SYNCHRONIZE,
};
...
/// `ActiveObserve` → `active_identity_changed`, `DestinationProbe` →
/// `talent_directory_io` (existence-probe I/O, never `already_claimed`),
/// `Write` → `write_io`, `Rename` → `completion_io`.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
fn checkpoint_traced(primitive: CortexAdmissionPrimitive) -> Result<(), CortexAdmissionError> {
    ...
    if fault {
        return Err(CortexAdmissionError::new(match primitive {
            ...
            CortexAdmissionPrimitive::ActiveObserve => CortexAdmissionClass::ActiveIdentityChanged,
    ...
    match stat_entry(talent, name) {
        Ok(Some(entry))
            if entry.kind == JournalEntryKind::RegularFile
                && identity_from_entry(&entry) == expected =>
        {
            Ok(())
        }
...
#[cfg(unix)]
fn identity_from_entry(entry: &FlatDirectoryEntry) -> CortexUseFileIdentity {
    CortexUseFileIdentity {
        unix: (entry.device as libc::dev_t, entry.inode as libc::ino_t),
    }
}
...
fn exclusive_write(
    ...
    Ok(CortexUseFileIdentity { windows: identity })
}
