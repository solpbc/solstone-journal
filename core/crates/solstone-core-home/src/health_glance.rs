...
pub fn build_health_glance(
    ...
    if status == "active" {
        ...
        json!({"verdict":"ok","severity":"green","headline":"no devices are running sol yet. set one up to start your journal.","last_observation":null,"cta":{"text":"set one up →","href":"/app/observer/"},"issues":[]})
    }
}
