...
- No journal access is available: no `solstone` command line, no read tools, no submit tools, no shell commands, no browser or web access, and no delegating to sub-agents.
...
mod tests {
    ...
    fn runtime_preamble_command_sentence_is_derived_from_vocabulary() {
        let sentence = format!(
            "- The approved host command families are {}; run them directly as `journal <family> ...` through the same tool, never prefixed with `solstone` or `solstone call`.\n",
