...
fn lane_env(
    ...
    let lane_dir = wrapper_dir.join(zig_target);
    let cc_wrapper = lane_dir.join("cc");
    let cxx_wrapper = lane_dir.join("c++");
    let ar_wrapper = lane_dir.join("ar");
    let ranlib_wrapper = lane_dir.join("ranlib");
