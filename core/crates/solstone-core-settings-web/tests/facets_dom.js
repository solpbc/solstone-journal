...
(async () => {
  ...
  await testCase('G3-107 notifications guide card links to the health log with a description and an action', async () => {
    const harness = createHarness();
    ...
    assert.ok(notifRow.querySelector('.sapp-desc').textContent.trim().length > 0, 'the card carries a description');
    assert.ok(notifRow.querySelector('.sapp-open').textContent.trim().length > 0, 'the card carries an action label');
  });
  ...
  await testCase('G3-116 storage/facets show a loading state while their reads are in flight', async () => {
    const harness = createHarness();
    assert.ok(
      harness.document.getElementById('storageLoadState').textContent.trim().length > 0,
      'storage must not be a blank pane while its read is in flight'
    );
    assert.ok(
      harness.document.querySelector('#facetsList .text-muted').textContent.trim().length > 0,
      'facets must not be a blank list while its read is in flight'
    );
  });
