...
/// Build the terminal record for a valid input with no decodable audio.
pub(crate) fn empty_record(input_size: u64) -> Value {
    record(
        vocab::STATE_EMPTY,
...
mod tests {
    ...
    use super::{analyzed_record, corrupt_input_record, empty_record};

    #[test]
    fn records_use_transcribe_vocabulary_without_attempts() {
        let records = [empty_record(4), corrupt_input_record(5), analyzed_record(6)];
        let expected = [
            (vocab::STATE_EMPTY, vocab::REASON_NO_DECODABLE_AUDIO, 4),
