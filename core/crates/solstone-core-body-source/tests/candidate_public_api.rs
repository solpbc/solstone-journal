...
fn assert_error(error: &CandidateError) {
    ...
    assert_eq!(error.coordinate.line(), 7);
...
fn public_api_projects_and_recovers_valid_and_refused_rows() {
    ...
    let valid_row = PresentationRow::from(parse(valid_text.as_bytes()).expect("valid row parses"));
    ...
    let invalid_row =
        PresentationRow::from(parse(invalid_text.as_bytes()).expect("invalid row parses"));
