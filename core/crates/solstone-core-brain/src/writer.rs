...
fn hold_record_lock(path: &Path) -> Result<solstone_core_journal_io::FileLock, WriterError> {
    hold_lock(
        path,
        LockOptions {
            mode: Some(BRAIN_FILE_MODE),
...
mod tests {
    ...
    #[test]
    fn crash_after_begin_keeps_checking_record_and_kernel_releases_lease() {
        let journal = TestJournal::new();
        journal.write_config("lane_byo_cloud");
        ...
        let config = projection_fixture().configs["lane_byo_cloud"]
            .as_object()
            .unwrap();
        let inspection = crate::inspect::inspect_brain_state(journal.path(), config, fixture_now());
        assert_eq!(
    ...
    fn begin_pause_helper() {
        let Ok(journal) = std::env::var("BRAIN_WRITER_BEGIN_PAUSE_JOURNAL") else {
            return;
        };
        let permit = begin_refresh(
            Path::new(&journal),
        ...
        .expect("permit");
        fs::write(
            std::env::var("BRAIN_WRITER_BEGIN_PAUSE_READY").unwrap(),
            b"ready",
        )
        .unwrap();
        let _permit = permit;
    ...
    fn renewal_pause_helper() {
        let Ok(journal) = std::env::var("BRAIN_WRITER_RENEWAL_PAUSE_JOURNAL") else {
            return;
        };
        ...
        fs::write(
            std::env::var("BRAIN_WRITER_RENEWAL_PAUSE_READY").unwrap(),
            b"ready",
        )
        .unwrap();
        let _permit = permit;
    ...
    fn refresh_contention_returns_no_permit_while_renewal_returns_busy() {
        ...
        journal.write_fixture_key();
        journal.seed_record("lane_spp/ready");
        let ready = journal.path().join("renewal.ready");
    ...
    fn record_writes_serialize_behind_a_real_process_lock() {
        ...
        let ready = journal.path().join("record-lock.ready");
    ...
    fn record_lock_pause_helper() {
        let Ok(journal) = std::env::var("BRAIN_WRITER_RECORD_LOCK_PAUSE_JOURNAL") else {
            return;
        };
        let path = brain_state_path(Path::new(&journal));
        let _lock = hold_record_lock(&path).unwrap();
        fs::write(
            std::env::var("BRAIN_WRITER_RECORD_LOCK_PAUSE_READY").unwrap(),
            b"ready",
        )
        .unwrap();
        loop {
    ...
    #[test]
    fn key_race_helper() {
        let Ok(journal) = std::env::var("BRAIN_WRITER_KEY_RACE_JOURNAL") else {
            return;
        };
        let key = generate_fingerprint_key(Path::new(&journal)).unwrap();
        fs::write(
            std::env::var("BRAIN_WRITER_KEY_RACE_RESULT").unwrap(),
            key.iter()
    ...
    fn key_generation_race_keeps_one_key() {
        let journal = TestJournal::new();
        let first = journal.path().join("first.key");
        let second = journal.path().join("second.key");
        let mut left = helper(journal.path(), &first).spawn().unwrap();
        let mut right = helper(journal.path(), &second).spawn().unwrap();
        left.wait().unwrap();
        right.wait().unwrap();
        let disk = fs::read(brain_fingerprint_key_path(journal.path())).unwrap();
        assert_eq!(disk.len(), FINGERPRINT_KEY_BYTES);
        let expected = disk
            .iter()
            .map(|byte| format!("{byte:02x}"))
            .collect::<String>();
    ...
    fn helper(journal: &Path, result: &Path) -> Command {
        let mut command = Command::new(std::env::current_exe().unwrap());
        command
            .args(["--exact", "writer::tests::key_race_helper", "--nocapture"])
            .env("BRAIN_WRITER_KEY_RACE_JOURNAL", journal)
            .env("BRAIN_WRITER_KEY_RACE_RESULT", result);
        command
    }

    fn begin_pause_command(journal: &Path, ready: &Path) -> Command {
        let mut command = Command::new(std::env::current_exe().unwrap());
        command
            .args([
                "--exact",
                "writer::tests::begin_pause_helper",
                "--nocapture",
            ])
            .env("BRAIN_WRITER_BEGIN_PAUSE_JOURNAL", journal)
            .env("BRAIN_WRITER_BEGIN_PAUSE_READY", ready);
        command
    }

    fn renewal_pause_command(journal: &Path, ready: &Path) -> Command {
        let mut command = Command::new(std::env::current_exe().unwrap());
        command
            .args([
                "--exact",
                "writer::tests::renewal_pause_helper",
                "--nocapture",
            ])
            .env("BRAIN_WRITER_RENEWAL_PAUSE_JOURNAL", journal)
            .env("BRAIN_WRITER_RENEWAL_PAUSE_READY", ready);
        command
    }

    fn record_lock_pause_command(journal: &Path, ready: &Path) -> Command {
        let mut command = Command::new(std::env::current_exe().unwrap());
        command
            .args([
                "--exact",
                "writer::tests::record_lock_pause_helper",
                "--nocapture",
            ])
            .env("BRAIN_WRITER_RECORD_LOCK_PAUSE_JOURNAL", journal)
            .env("BRAIN_WRITER_RECORD_LOCK_PAUSE_READY", ready);
        command
    }
