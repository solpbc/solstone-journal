...
- `add`: Use when the suggestion is a durable fact not previously known, or a fact that changed in the world (a relationship that evolved, a role that moved). Older observations remain in place. The store automatically keeps a suggestion out if a live observation already matches it on content, day, and relation — it does not catch a repeat on a different day or under a different relation, so still prefer `skip` when an existing observation already covers the fact.
