...
"""CDP browser scenario verification.
...
_SNAPSHOT_JS = r"""
(() => {
  window.__solstoneRefs = window.__solstoneRefs || new Map();
  window.__solstoneRefIds = window.__solstoneRefIds || new WeakMap();
  window.__solstoneNextRef = window.__solstoneNextRef || 1;
  ...
  const labelText = (el) => {
    const direct = Array.from(el.labels || []).map((label) => label.innerText).join(' ');
    const labelledBy = norm((el.getAttribute('aria-labelledby') || '').split(/\s+/)
      .map((id) => document.getElementById(id)?.innerText || '').join(' '));
    return norm(direct || labelledBy);
  };
  const refFor = (el) => {
    let ref = window.__solstoneRefIds.get(el);
    if (!ref) {
      ref = `e${window.__solstoneNextRef++}`;
      window.__solstoneRefIds.set(el, ref);
    }
    window.__solstoneRefs.set(ref, el);
    return ref;
  };
  ...
  document.querySelectorAll('*').forEach((el) => {
    ...
    const role = norm(el.getAttribute('role') ||
      (tag === 'textarea' ? 'textbox' : tag === 'input' ? (type === 'search' ? 'searchbox' : 'textbox') : ''));
    const name = norm(el.getAttribute('aria-label') || el.getAttribute('name') ||
      el.getAttribute('title') || el.getAttribute('placeholder'));
    const label = labelText(el);
    const value = 'value' in el ? norm(el.value) : norm(el.getAttribute('value'));
    const text = norm(el.innerText || el.textContent);
    if (!role && !name && !label && !value && !text && !['input','textarea','select','button','a'].includes(tag)) return;
    nodes.push({ref: refFor(el), role, tag, name, text, label, value});
  });
  return {nodes};
})()
"""
...
# exposes __solstoneVisualBeforeCapture for deterministic capture.
...
DEFAULT_VIEWPORT = {"width": 1280, "height": 713}
...
def launch_pinchtab(port: int, timeout: int = 30) -> subprocess.Popen:
    env = {
        **os.environ,
        "PINCHTAB_PORT": str(port),
    ...
    stderr_path = f"/tmp/pinchtab-{port}.log"
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            try:
                ...
                stderr = ""
            raise RuntimeError(
                f"pinchtab exited with code {process.returncode}\n{stderr}"
            )
        try:
            health = _get_json(f"http://localhost:{port}/health", timeout=2)
            instance = health.get("defaultInstance") or {}
            if health.get("status") == "ok" and instance.get("status") == "running":
                return process
        except (OSError, TimeoutError, json.JSONDecodeError):
            pass
        time.sleep(0.5)
    stop_pinchtab(process, port)
    raise RuntimeError("pinchtab failed to start")


def stop_pinchtab(process: subprocess.Popen, port: int) -> None:
    instance_ports = _pinchtab_instance_ports_from_log(port)
    pid = process.pid
...
class CdpPage:
    ...
    def evaluate_json(self, expression: str, *, timeout: float = 10.0) -> Any:
        ...
        if not isinstance(raw, str):
            raise CdpError(f"Runtime.evaluate returned non-JSON value: {raw!r}")
        try:
            return json.loads(raw)
        except json.JSONDecodeError as exc:
            raise CdpError(f"Runtime.evaluate returned invalid JSON: {raw!r}") from exc

    def text(self, *, timeout: float = 10.0) -> str:
        value = self.evaluate("document.body.innerText", timeout=timeout)
        return "" if value is None else str(value)

    def assert_text(self, expected: str, *, timeout: float = 10.0) -> bool:
        return expected.lower() in self.text(timeout=timeout).lower()

    def snapshot(self, *, timeout: float = 10.0) -> dict[str, Any]:
        snapshot = self.evaluate_json(_SNAPSHOT_JS, timeout=timeout)
        if not isinstance(snapshot, dict) or not isinstance(
            snapshot.get("nodes"), list
        ):
            raise CdpError(f"snapshot returned invalid shape: {snapshot!r}")
        return snapshot

    def type_ref(self, ref: str, text: str, *, timeout: float = 10.0) -> None:
        self.evaluate(
            f"window.__solstoneRefs.get({json.dumps(ref)}).focus()",
            timeout=timeout,
        )
        self._connection.call("Input.insertText", {"text": text}, timeout=timeout)
    ...
    def capture_png(self) -> bytes:
        return self.capture_screenshot("png")

    def capture_screenshot(self, image_format: str) -> bytes:
        result = self._connection.call(
            "Page.captureScreenshot",
            {"format": image_format, "fromSurface": True},
            timeout=20,
        )
    ...
    def collect_console_errors(self) -> list[str]:
        try:
            parsed = self.evaluate_json("window.__pt_errors || []")
        except CdpError:
            return []
...
def _extract_detail_path(page: CdpPage, rule: str) -> str | None:
    values = page.evaluate_json(DETAIL_HREF_JS)
...
def _eval_json(page: CdpPage, expression: str) -> Any:
    return page.evaluate_json(expression)


def _assert_loading_cleared(page: CdpPage, path: str) -> list[str]:
    try:
        if _eval_json(page, "!document.querySelector('[data-loading]')"):
            return []
    except Exception as exc:
        return [f"loading sentinel check failed for {path}: {exc}"]
    return [f"loading sentinel still visible: {path}"]


def _resolve_route_path(
    page: CdpPage, base_url: str, rule: str
    ...
    if _route_regex(rule).fullmatch(parent):
        return parent, None
    page.navigate(_resolve_url(base_url, parent, capture=True))
    time.sleep(1.2)
    path = _extract_detail_path(page, rule)
    if path:
        return path, None
...
def run_cdp_scenario(
    ...
    try:
        viewport = scenario.get("viewport", DEFAULT_VIEWPORT)
        ...
        # -> capture image -> close the target in finally.
        page.add_script_on_new_document(_ERROR_LISTENER_JS)
        ...
        for step in scenario["steps"]:
            ...
            if action == "navigate":
                ...
                if not page.assert_text(step["text"]):
                    errors.append(f"assert_text failed: {step['text']!r} not in body")
                ...
                if last_snapshot is None:
                    raise CdpError("find_input before snapshot")
                ...
                ref = variables.get(var_name)
                if not ref:
                    raise CdpError(f"type step missing variable {var_name!r}")
                page.type_ref(ref, step["text"])
                ...
                if scenario.get("diff"):
                    image = page.capture_png()
                else:
                    image = page.capture_screenshot("jpeg")
                if mode == "update":
                ...
                if not path.exists():
                    ...
                    continue
                if not scenario.get("diff"):
                    continue
                result = compare_png(image, path.read_bytes())
                if not result.passed:
                    actual_path, diff_path = _write_diff_artifacts(path, image, result)
                    errors.append(
...
def run_cold_load_smoke(cdp: CdpBrowser, base_url: str) -> list[dict[str, Any]]:
    ...
    for rule in _derive_app_page_routes():
        ...
        try:
            page = cdp.create_page("about:blank")
            page.add_script_on_new_document(_ERROR_LISTENER_JS)
            path, skip_reason = _resolve_route_path(page, base_url, rule)
            if skip_reason:
            ...
            if not skipped:
                page.navigate(_resolve_url(base_url, path, capture=True))
                time.sleep(1.2)
...
def run_all(
    ...
    if not scenario_filters:
        for result in run_cold_load_smoke(cdp, base_url):
    ...
    for scenario in selected_scenarios:
        ...
        result = run_cdp_scenario(cdp, scenario, base_url, mode)
...
def main(argv: list[str] | None = None) -> int:
    ...
    process = launch_pinchtab(args.pinchtab_port)
    cdp: CdpBrowser | None = None

    try:
        cdp = CdpBrowser.from_pinchtab_port(args.pinchtab_port)
        logger.info("Running browser scenarios (%s)...", args.command)
        try:
            results, console_errors = run_all(
                cdp, args.base_url, args.command, args.scenario
            )
        except ValueError as exc:
            logger.error("%s", exc)
            return 2
        ...
        if cdp is not None:
            cdp.close()
        stop_pinchtab(process, args.pinchtab_port)
