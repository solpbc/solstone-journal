...
| Entity merge/undo interruption records (`health/entity-merge-recovery/*`) | `core/crates/solstone-core-entity/src/store/merge_rollback.rs`; merge/undo retry repairs committed derived work or restores unchanged checkpointed sources. Conflicts retain current files and before-images. Undo refuses changed merged facet files and older payloads without recorded after-state. |
