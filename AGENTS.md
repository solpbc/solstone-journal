...
| Timeline (`chronicle/<day>/timeline.json`, `chronicle/**/<seg>/timeline.json`, root `timeline.json`) | Segment artifacts: `core/crates/solstone-core-timeline/src/{store,binding}.rs`. Day/master artifacts: `core/crates/solstone-core-maintenance/src/bodies/timeline.rs` (`rollup_day`, `rollup_master`) pending their commits 5–6 migration. `solstone-core-talent-runtime`'s `timeline:segment_summary` stage calls the timeline crate directly as an orchestration caller, not a separate writer. |
