...
| — | ⛔ **No gate launches a browser.** `make ci` is unit tests only; `make ci-full` is minimal integration on local tools. Front-end JavaScript is tested under plain Node with a stand-in DOM (the `shell_chrome_dom` pattern); the HTML pages under `assets/*/tests/` are opened by hand. `make_gates_never_launch_a_browser` enforces it. |
