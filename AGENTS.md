...
| Facets (`facets/*/facet.json`, `facets/*/relationships/`) | `core/crates/solstone-core-facets/src/store/write.rs` — full lifecycle: create/update/mute/delete/rename plus relationship link/detach (create allocates a journal-unique `id`). A second, legitimate producer, `core/crates/solstone-core-import-web/src/facet_ingest.rs`, writes `facet.json` directly for bulk journal-archive import/sync, a different flow (bulk import vs. interactive) rather than competing ownership (allocates a new local `id` and never adopts an incoming `id`). |
