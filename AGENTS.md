...
> If you cannot state in one sentence **which module owns the data your change touches**, stop and re-read §7 L2 (the domain ownership table). Writing to a domain from the wrong module violates that boundary.
...
| `make skills` | Refresh the `solstone` + `journal` router skill symlinks in `journal/`. |
...
| `make test` | Focused code-test path for contributors. It runs selected library and binary unit harnesses, including the runtime-free same-crate units in the three ONNX-linked packages through their no-default-feature library closure, then prints its source-derived evidence boundary. |
...
| `make ci` | Routine code-landing gate: formatting, CI-topology validation, library and binary Clippy, and the same selected unit harnesses as `make test`. For `solstone-core-speakers-analyze`, `solstone-core-speakers-onnx`, and `solstone-core-vad-analyze`, it statically checks the default production closure and runs the runtime-free library closure. Their broader same-crate tests use `full-tests` with the normal runtime feature and remain in the staged full gate. In every crate, tests that start a process, open a socket, read host state or wait on the wall clock belong behind that crate's `full-tests` feature, which `make ci-full` runs. |
| — | ⛔ **No gate launches a browser.** `make ci` is unit tests only; `make ci-full` is minimal integration on local tools. Front-end JavaScript is tested under plain Node with a stand-in DOM (the `shell_chrome_dom` pattern); the HTML pages under `assets/*/tests/` are opened by hand. |
...
| `WIN_REMOTE_HOST=user@host make win-host-ci` | Transfer an exact, source-bound snapshot to the configured Windows build host and run the first native MSVC journal substrate gate. It first acquires the four pinned FFmpeg build-toolchain archives with `solstone-distribution acquire ffmpeg-windows-tools` and transfers any the host does not already hold, because the host carries no ambient MSYS2 shell, GNU make, NASM or libclang and cannot acquire them itself; the gate re-verifies them against the transferred checkout's own pins, stages them, and reports the four archive identities plus `JOURNAL_WIN_CI_FFMPEG_TOOLS=executed/pass`. It builds `solstone-core-journal`, `solstone-core-journal-config`, `solstone-core-journal-io`, `solstone-core-system`, and `solstone-core-win-owner-rail`; compiles the `solstone-core` library test harness with `test-hooks`; runs journal-config, journal-io (library, lock-component, detailed-atomic-publication), and the required journal library tests; then proves NTFS and ReFS publication, Cortex-use recovery, canonical operational-log discovery, and stale-heartbeat cleanup through separate native receipt children, plus the mandatory ordinary-owner inventory control. The final `JOURNAL_WIN_HOST_CI_VERIFIED` line reports each of those as `executed/pass`; Cloud Files sync-root registration is the one leg that stays opt-in and reads `skipped` unless `JOURNAL_WIN_CI_RUN_CLOUD_SYNC_TEST=1`. |
| `make verify` | Alias for `make ci`. |
| `make build-native-sol-inventory` | Regenerate the compiled `solstone call` inventory after an `authority.toml` change. A Rust `make ci` test refuses stale authority sources. `make check-native-sol-inventory` remains a manual exact-output check. |

Use the narrowest applicable `make check-rust-*` target for a focused change, then `make test` for selected unit evidence and `make ci` before commit. Run `make ci-full` on the exact final-tree SHA before merge or release for full host evidence.
...
only after the remote checkout acknowledges both values. Its one FFmpeg-bearing
step is the runtime-component receipt near the end of the gate, so a build host
missing the pinned toolchain used to fail only after everything before it had
already passed; the toolchain is staged and proved up front for that reason. Treat the runner's explicit
...
The three ONNX-linked packages have two explicit Cargo closures. Their default `runtime` feature is the production closure used by shipped helpers and supported-target compilation. Their no-default-feature library closure contains only runtime-free code and same-crate unit tests. The nondefault `full-tests` feature selects broader same-crate tests and activates the same `runtime` feature already present in production defaults. It is not an alternate runtime or a shipping feature.

Use `cargo test --manifest-path core/Cargo.toml -p <package> --no-default-features --lib` for a focused runtime-free unit run in one of these packages. Use `make check-rust-onnx-test` for their staged, serial full same-crate tests. `solstone-core-vad-analyze::vad_oracles` remains a separate Cargo integration target selected by the full registry.

- Put same-crate behavior tests in `#[cfg(test)]` modules beside the owning code. Cargo's target classification (`--lib`/`--bins` versus a `--test` binary) is a build-cost decision, not a claim about validation scope. The routine library/binary gate is for code correctness, contract correctness and build cleanliness. A test that starts a process, opens a socket, reads host state or waits on the wall clock goes behind the crate's non-default `full-tests` feature: mark the test function or module `#[cfg(all(test, feature = "full-tests"))]`, declare `full-tests = []` in the crate manifest, and enable it on the crate's package suite in `core/ci/suites.toml` (`features = ["full-tests"]`, `default_full = true`). CI topology validation refuses a package that declares the feature without a suite running it. A test's location under `src/` does not make it routine evidence. Keep a mixed module wholly broad or split it only when the smaller group has a coherent boundary of its own. Put public API and process contracts in the narrowest owning leaf crate unless the behavior genuinely belongs to the aggregate `solstone-core` composition boundary.
...
- Put repository, source, manifest, Makefile, packaging, and workspace-policy
  ...
  placeholder.
- A new check that walks the whole repository belongs in `make ci-full`. Prefer
  a visibility boundary the compiler enforces to a scan, and do not pin the text
  of the Makefile or CI configuration in a test.
- Do not write tests that enforce design choices such as copy, casing, punctuation or color.
- Iterate with the narrowest command, such as `cargo test --manifest-path core/Cargo.toml -p <package> --lib` or the affected `--test <harness> <test>`. A default-feature `--lib`/`--bins` selection contains only routine same-crate evidence; an explicit `--test` harness keeps its declared validation scope. For a crate's `full-tests` evidence, run `make check-rust-registry-package CI_PACKAGE=<package> CI_FEATURES=<features> CI_RUNTIME=<runtime>` with the features and runtime its package suite in `core/ci/suites.toml` names; `make check-rust-clippy-full` lints the same feature sets. `make ci` is the routine gate: a formatting check, topology validation, library/binary Clippy, and library/binary unit harnesses. It does not link or execute integration-test binaries, so run affected harnesses directly.
...
⚠ **L1/L2 domain-boundary discipline has no automated grep check today.** The old low-bar checkers, `scripts/check_layer_hygiene.py`, `scripts/check_journal_io_access.py`, `scripts/check_journal_io_mechanic.py`, `scripts/check_api_conventions.py` and `scripts/check_journal_config_owner.py`, read the Python tree the conversion deleted; they were removed rather than left passing vacuously. L1/L2 are held by review and by Rust module/crate boundaries, not by a gate. **L8 is the exception** — see below.
...
| Push VAPID key (`config/push-vapid.json`) | `core/crates/solstone-core-push/src/vapid.rs` |
...
| Native speakers-analyze install generation (`health/speakers-analyze/install-generation.json`, `.lock`) and its additive, content-free owner diagnostic (`health/speakers-analyze/owner.json`) | `core/crates/solstone-core-transcribe/src/speakers_installation.rs` (`enter_speakers_analyze_generation`; the owner file is written only on acquire, never consulted for acquire/borrow authorization) |
...
| Raw-media offload audit (`chronicle/<day>/health/oplog--offload~tag--…--raw-media-offload~tag.jsonl`) | `core/crates/solstone-core-offload/src/pruning_audit.rs` opens one canonical `offload` / `raw-media-offload` oplog per invocation and appends one row per marked segment. Historical `health/pruning-runs/<YYYYMMDD>.jsonl` files have no current writer; `solstone-core-retention/` only ages them out under its retention policy. |
...
| Stream identity (`chronicle/**/<seg>/stream.json` marker + `streams/<name>.json` state) | `core/crates/solstone-core-segment` (`bind_stream` / `bind_named_stream` / `bind_paired_stream` / `advance_unbound_stream` / `advance_bound_stream`); observer prune repairs a survivor's predecessor pointers locally |
| Observer ingest manifest (`chronicle/**/<seg>/ingest.json`) | `core/crates/solstone-core-ingest-resolve` (`write_ingest_manifest`) |
| Link CA and client authorization state (`link/ca/cert.pem`, `link/ca/private.pem`, `link/ca-staging/**`, `link/state.json`, `link/authorized_clients.json`) | `core/crates/solstone-core-sol-link/src/establish.rs` (CA, staging, state) and `.../src/ledger.rs` (`authorized_clients.json`). `solstone-core-convey-shell` never writes these paths itself: it reads via `read_authorized_clients` and routes every removal (`clients.rs::delete_client`, `network.rs::unpair`, `network_writes.rs::forget_device`) through the owner's `AuthorizationLedger`. |
| Link client descriptions (`link/client-descriptions.json`) | `core/crates/solstone-core-sol-link/src/client_description_store.rs` (`put_self_description`, `patch_owner_label`). HTTP routes in `solstone-core-convey-shell` are callers only. `AuthorizationLedger::remove` cascades deletions under the auth lock. |
| Link account token file (`link/tokens/account.json`) | `core/crates/solstone-core-spl/src/private_link.rs::save_service_token`; `link_state_files.rs` is its read-only consumer. |
...
| MCP endpoint owner identity, durable PoP key, bearer-token verifier ledger, OAuth ledger, and connection permissions (`mcp-endpoint/**`, including `tokens.json`, `oauth.json`, and `permissions.json`) | `core/crates/solstone-core-mcp-endpoint/` (`lib.rs::bootstrap_mcp_endpoint_owner_identity`, `unix.rs`, `tokens.rs`, `oauth/store.rs`, `permissions.rs`) |
| MCP agent interaction audit records (`chronicle/**/mcp.agent/<seg>/{interaction.json,outcome.json}`) | `core/crates/solstone-core-mcp-audit/` (`write_interaction_record` publishes the admission before the tool runs; `write_outcome_record` publishes its immutable sibling after the response is approved for release). Both are create-exclusive and neither is ever amended — an admission with no outcome means the call's end is genuinely unknown. The crate is a **write-only, dependency-pure leaf**: the owner's read is `solstone-core-mcp-endpoint/src/activity.rs`, and nothing else writes this stream. ⛔ The supervisor deliberately does **not** enrich it (`supervisor/tick.rs::is_mcp_audit_segment`). |
| Chronicle day content (`chronicle/YYYYMMDD/**`) | The capturing module (observer, importer) per its declared outputs |
| Segment durable events (`chronicle/**/<seg>/events.jsonl`) | Mixed durable log written through `solstone_core_callosum::append_durable_event` by ingest, sense, transcribe, the supervisor, and the retention crate's raw-original deletion (`core/crates/solstone-core-retention/src/door.rs`, `release_raw`). Only the supervisor's rows copy bus events; the other writers append directly. |
| Index (SQLite, `indexer/*`) | Schema, connection, writes, and classification sidecar tables (computed by `classification.rs` and written beside index rows): `core/crates/solstone-core-indexer-store/`. What to persist for discovery, edges, entity search, and metadata: computed by `core/crates/solstone-core-indexer/`, which calls into indexer-store. Query surface: `core/crates/solstone-core-indexer-query/` is read-only. |
...
| Route repair coordination lock (`<prefix>/.solstone-route.lock/**`) | **External shell callers only** (the future installer) create and remove the directory and owner file. `core/crates/solstone-core/src/journal_route/coordination_lock.rs` owns validation and the on-disk format contract only; `journal_route::repair` never creates, acquires, or removes this path. |
...
- **Keep `make ci` unit-only.** A new routine test checks code correctness or a contract (a wire format, a schema, a function's documented output). A test that starts a process, opens a socket, reads host state or waits on the wall clock goes behind its crate's `full-tests` feature, which `make ci-full` runs. The CI topology check refuses a direct process spawn or socket constructor in routine test code, but it cannot see one behind a helper, so the rule is yours to keep. Put a test that walks the whole repository in `make ci-full`, and prefer a visibility boundary the compiler enforces to a scan. Do not write a test that pins Makefile or CI text, or one that enforces copy, casing, punctuation, color or other design choices. Mechanics: §5 Rust test topology and `docs/testing.md`.
