...
| `scripts/` | Repo maintenance scripts, mostly Python: generators, release and Windows build helpers, and four Python checks reached through `make check-native-sol-inventory`, `make check-skill-references`, `make check-native-sol-architecture` and `make check-spl-dependency-pin`. No gate runs those four, because `make ci` and `make ci-full` both forbid Python | tooling that guards the codebase | `docs/SOLCLI.md` |
...
| `make ci` | Routine code-landing gate: formatting, CI-topology validation, library and binary Clippy, and the same selected unit harnesses as `make test`. On Linux, it also runs the convey page runner. For `solstone-core-speakers-analyze`, `solstone-core-speakers-onnx`, and `solstone-core-vad-analyze`, it statically checks the default production closure and runs the runtime-free library closure. Their broader same-crate tests use `full-tests` with the normal runtime feature and remain in the staged full gate. |
...
| `make check-skill-references`, `make check-native-sol-inventory` | Check that the generated skill references and the native sol inventory match their sources. They are Python, so no gate runs them: run both after changing an `authority.toml` or a skill fragment. |
...
| Speaker discovery clusters, resolved (`awareness/discovery_clusters.resolved.json`) | `core/crates/solstone-core-speaker-resolve/src/identify_forward_phases.rs` (`replace_resolved_clusters`), a different module from the unresolved file above. `solstone-core-entity` invalidates the unresolved cache after committing merge/undo source changes; it does not snapshot or restore either cache. |
