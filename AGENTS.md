...
| Timeline (`chronicle/<day>/timeline.json`, `chronicle/**/<seg>/timeline.json`, root `timeline.json`) | Segment/day artifacts: `core/crates/solstone-core-timeline/src/{store,binding}.rs`; `solstone-core-maintenance/src/bodies/timeline.rs` orchestrates day curation and calls that owner. Master artifacts remain in `core/crates/solstone-core-maintenance/src/bodies/timeline.rs` (`rollup_master`) pending commit 6. `solstone-core-talent-runtime`'s `timeline:segment_summary` stage calls the timeline crate directly as an orchestration caller, not a separate writer. |
