...
| Speaker labels (`chronicle/**/talents/speaker_labels.json`) | `core/crates/solstone-core-speaker-resolve/` via `solstone-core speaker-resolve <verb>`; `solstone/apps/speakers/native.py` is the sole Python transport. `solstone/apps/speakers/attribution.py` and `routes.py` prepare requests only; `attribution.py` remains only for the Lane B entity-merge flow through `update_speaker_labels`. |
| Speaker corrections (`chronicle/**/talents/speaker_corrections.json`) | `core/crates/solstone-core-speaker-resolve/` via `solstone-core speaker-resolve <verb>`; `solstone/apps/speakers/native.py` is the sole Python transport. `solstone/apps/speakers/attribution.py` and `routes.py` prepare requests only; `attribution.py` remains only for the Lane B entity-merge flow through `remap_speaker_corrections_for_entity_merge` and `apply_entity_merge_segment_inverse`. |
...
| Health import local state (`imports/health-dedupe.sqlite`, `imports/_approvals/**`, Apple Health `imports/<id>/{raw,normalized,content_manifest.jsonl,manifest.json}`) | `solstone/think/importers/{apple_health,health_dedupe,pre_save_gate,shared}.py` — Apple Health save-mode reads the approval artifact before any write, installs raw synthetic/approved sources under the import bundle, writes normalized shards under `imports/<id>/normalized/`, and keeps record dedupe in `imports/health-dedupe.sqlite`. The pre-save gate is read-only; creation of `_approvals/**` remains a separately approved setup action. |
