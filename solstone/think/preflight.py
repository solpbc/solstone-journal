...
This battery can run before `.venv` or `uv` exist. It is an intentional,
temporary duplication of a subset of `solstone/think/doctor.py`; a future lode
will unify doctor and preflight onto shared stdlib code. See doctor.py's
decision-log for the source rationale.
...
# These constants and the check bodies below are duplicated verbatim from
# `solstone/think/doctor.py` (see its decision-log for the MIN_UV=0.7.12 and
# MIN_FREE_GIB=10 rationale). Keep them in sync until the doctor/preflight unify
# lode reconciles them.
