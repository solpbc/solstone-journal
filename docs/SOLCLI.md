...
4. **Update the app command fragment** if it documents the changed behavior.
   These optional references live at `core/payload/solstone/apps/*/talent/*/SKILL.md`:
...
5. **Update the `solstone` router's short app map** if this is a new app agents
   should use. The command list comes from `solstone call <app> --help`.

6. **Run `make skills`** to refresh the installed `solstone` + `journal`
   router skill symlinks.
...
3. **Optionally update a router skill** if agents need additional guidance.
...
| `core/payload/solstone/apps/<name>/talent/<name>/SKILL.md` | Optional app reference guidance | If it documents the changed behavior |
| `core/payload/solstone/talent/solstone/SKILL.md` | Short routing map for agent use | When adding an app agents should use |
...
Use the journal-config transactional surface to preserve the file's locking and
compare-and-swap protection: read the current object with `solstone-core
journal-config read [--journal PATH]`, make the change to its `config` object,
then send the complete replacement object on standard input to `solstone-core
journal-config commit [--journal PATH] --expect sha256:<value returned by
...
| Specialized | `config`, `mcp` ([endpoint](#journal-mcp-endpoint)), `skills`, `streams`, `journal-stats`, `reprocess`, `formatter` |
| Installation | `doctor` |
| Help | `help` |
...
`solstone skills` manages coding-agent skill installation with `install`, `uninstall`, and `list`. The former `build` verb is gone; `make skills` invokes `solstone skills install`.
...
There is no in-repo `vit` skill. App fragments document behavior beyond what
`--help` shows (e.g., checking entity context before attaching a new
relationship). The two installed router skills give a short route to the right
app and direct agents to live CLI help for its current commands and flags.

### Keeping skills in sync

When you add or change a `solstone call` command, update the native authority,
handler, and any app fragment that describes its behavior. Include:
- Full command syntax with all flags
- Behavior notes (edge cases, defaults, validation)
- Examples showing common usage patterns

Run `make build-native-sol-inventory` after an authority change. `make ci`
checks that the committed compiled inventory tracks every authority file.
Run `make skills` after changing a router skill to refresh project symlinks.
