...
This design moves the three write paths for `health/brain.json` from
...
Derive `projection` by calling the shared validator's `inspect::inspect_brain_state` fresh
...
from the module this design empties, so a post-cut tree cannot regenerate these
...
> installed aggregate binary, not as a standalone writer binary.  This design also
> retires `scripts/local_contract_corpus.py` and
> `scripts/brain_projection_corpus.py` from `expected_outputs()`: together they
> import 42 attributes from `brain_state.py`, which this design reduces to a thin
...
This belongs beside the existing native dual-path doctrine and the
...
| 19 | `inspect` still creates nothing — record, key, either lock, lease. Re-assert with the same full-tree snapshot approach the shared validator used. | Keep `solstone-core-brain` inspection read-only; neither inspect nor composition acquires/creates record, key, state lock, or refresh lease. **Implementation attention:** re-run the shared validator's full-tree snapshot assertion against native inspect. |
...
| 27 | `core/fixtures/local_contract.json` and `core/fixtures/brain_projection.json` are byte-unchanged — assert via hash comparison against the base commit, not `git diff` against HEAD. | Treat both fixtures as frozen; implementation verification compares their hashes to the base commit. Do not use HEAD diff as the proof. |
...
| 30 | End-to-end verification through a real Python caller against a live journal is explicitly post-ship work — do not attempt it in this design or report it as done. | Explicitly out of scope. Implementation may run fixtures/process tests only; it must neither attempt live-journal verification nor claim it complete. |
