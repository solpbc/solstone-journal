...
| 2. Structured sources | `--source ics|obsidian|document|image|journal_archive|chatgpt|claude|gemini|kindle SOURCE` | Each route resolves the real input and reaches its native source body: preview-only sources render a preview only with `--dry-run` and otherwise refuse honestly; writing sources render their body result or a named body-specific dependency failure. |
