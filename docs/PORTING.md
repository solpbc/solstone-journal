...
as `WIN_REMOTE_HOST=user@host make win-host-ci`. It transfers an exact,
source-bound Git snapshot, verifies the workspace lockfile digest on the
Windows checkout, builds `solstone-core-journal` and
`solstone-core-journal-config` with MSVC, and runs the config crate's unit
tests. Its success line names the Windows behavior not covered by that run.
Do not treat this transport gate as evidence for journal filesystem I/O,
archive behavior, Callosum, packaging, installation, signing, or smoke tests.
