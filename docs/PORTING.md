...
The opt-in processing bundle is a direct rpath proof. After `make check-rust-onnx-stage`, run `make build-sandbox-processing` and then [`make check-rust-sandbox-processing-build`](../Makefile#L514) against the same Cargo target; the check proves both helpers [start without loader-path variables (`sandbox_processing_check_uses_only_the_existing_payload_and_clears_loader_paths`)](../core/crates/solstone-core-repository-contracts/tests/repository_make_command_graphs.rs#L750) and [performs no build or repair (`sandbox_processing_check_rejects_invalid_payload_before_helpers`)](../core/crates/solstone-core-repository-contracts/tests/repository_make_command_graphs.rs#L655). For the negative proof, copy a helper to a sibling-less scratch directory, clear both loader-path variables, and confirm the loader fails before a structured request error is emitted.
