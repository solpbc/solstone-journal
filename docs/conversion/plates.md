...
**Measured facts about four of those deferred apps, 2026-08-13** — engineering findings, not a grouping decision:
...
- ⚠ **`awareness` is API-only**: no `workspace.html`, no `app.json`, no registry row, and `GET /app/awareness/` is deliberately a 404. Its four routes exist to serve the native CLI, so it is not an owner-visible surface in the reserved sense.
