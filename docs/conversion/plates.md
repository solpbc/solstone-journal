...
⚠ **The durable format carries NO version and NO encoder identity, and the native reader hard-codes width 256** (`voiceprints.rs:30`). The encoder is named only in `apps/speakers/encoder_config.py`. 🔴 **An encoder change therefore makes every existing voiceprint unreadable with no migration path — which IS a forced re-teach.** The conversion adds an **additive** `envelope.npy` (format, version, encoder id + sha256, width) and relaxes the reader from exact-member-set to required-subset; **absence is legacy, never corrupt**, and the store self-heals on first write. ✅ Additive in both directions — Python's `load_npz` ignores extra members, so a v1 file stays readable by shipped `1.0.22`.
