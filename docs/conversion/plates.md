...
📌 **Current state, 2026-09-16.** Both raw-audio unlink sites above are gone, and native transcription never removes the raw audio: it writes the `empty` record and leaves the file. Retention's `empty_audio` setting decides whether that audio is marked for removal, and removal waits for the owner's approval. Transcription no longer reads `transcribe.preserve_all`; a stored `true` is treated as `retention.empty_audio: "keep"`.
...
✅ **DURABLE SPEAKER BACKFILL OVER SHORT HTTP CALLS (2026-09-18).** The backfill ledger (`speakers/backfill-operations.jsonl`) is updated to schema v3 with monotonic events (`accepted`, `attempt_started`, `prepared`, `checkpoint_batch`, `attempt_failed`, `completed`), backward-compatible with legacy v1/v2 rows without rewriting history. Ledger recovery enforces newline-delimited parsing, torn-tail load tolerance, and truncation under lock before writes. The background execution coordinator enforces `(ObjectIdentity, operation_id)` single-attempt owner tokens, takes the non-blocking execution lock `health/locks/speaker-backfill`, scans segments with a 4-way classification (`Absent`, `Stub`, `Protected`, `Gap`), runs per-member resolution units under entity-trust locking with optional voiceprint accumulation, and writes bounded checkpoint batches (1..=64 items). Invariants: (1) stub = empty `labels` array AND boolean `skipped:true`; protected = any other object with a `labels` array; gap fails before `prepared`; `reattribute:false` leaves protected bytes identical. (2) report-only uses the same durable path; operation rows are its only writes. Short HTTP handlers (`POST /app/speakers/api/backfill`, `GET /app/speakers/api/backfill/operations/{operation_id}`, `POST /app/speakers/api/backfill/operations/{operation_id}/resume`) and native CLI commands (`solstone call speakers backfill`, `backfill-status`, `backfill-resume`) separate operation lifecycle from network request lifetime.
