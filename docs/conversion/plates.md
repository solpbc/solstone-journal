...
🔴 **The covenant property lives here, and as of 2026-08-12 it is CONVENTIONAL, not structural.** ⚠ Corrected same-day: an earlier revision of this entry claimed a single-element host allowlist inside the fetch primitive. **There is none.** `ALLOWED_HOSTS` is 0 hits repo-wide, there is no `Artifact::origin_key`, and no CI ratchet guards the origin. `install::archive::download` takes a URL, calls `ureq::get(url)`, and verifies **only the sha256** — which proves *what* you received, never *where from*. The origin flip was **data-only**: the pin URLs in `install/pins.rs` and `solstone-core-assets` are `updates.solstone.app` literals, and nothing stops a future caller from passing an upstream URL with a correct digest.

⛔ **So "every download comes from our own cache" is true of today's pin data and is not enforced by anything.** Making it structural — a host check inside the one fetch primitive — is open work, not a property the plate already holds. ⚠ Do not cite this plate as evidence the covenant is mechanically guaranteed.

⚠ **A denylist would not deliver it either, and that part stands**: `github.com` 302s to `release-assets.githubusercontent.com` and `huggingface.co` to `us.aws.cdn.hf.co`, so a fallback reaching either makes zero requests to the denied names. The enforcement to build is an allowlist.

🔴 **The macOS MLX path still downloads from a third party today.** `providers/mlx_install.py` calls `huggingface_hub.snapshot_download` and `HfApi().list_repo_tree` directly, so an owner installing the local provider on Apple Silicon tells Hugging Face their journal exists. ⚠ **The native side cannot simply be ported onto it** — a Rust port of that call reproduces the violation. `install/mlx.rs` and `run_mlx_install` already cover validation and publication from a *local* snapshot; what has no native owner is the fetch. The 25-object snapshot set is mirrored and digest-verified at our origin, so the covenant-clean shape is fetch-ours → validate → publish.
