...
| `S:web:journal` | `P-journal` | ⛔ 100% of `P-web` is `localhost:5015` or an authorized linked device. ✅ **The localhost half is SERVED NATIVELY as of 2026-08-10** — `solstone-core convey` binds the loopback listeners and serves the shell, the app registry, the session gate and the speakers surface, with no Python in the request path (`P-web-speakers`, first `P-web-*` lane). ⚠ **The two halves now diverge and it is not symmetric:** the linked-device half reaches convey through `convey/secure_listener/`, which terminates TLS in Python and **dispatches inline into the Flask WSGI app** — so when the Rust process takes the port, that door dispatches into nothing and **linked devices go dark**. ⛔ Authorized breakage, not a defect, and ⛔ not `P-web`'s to repair: the listener is `P-device-link`'s authorization boundary and Python by ruling. 📌 Recorded here because a boundary that has quietly stopped working, with its code still on disk reading as live, is how a security surface rots |
