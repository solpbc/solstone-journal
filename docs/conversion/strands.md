...
⚠ The talent event vocabulary is **NDJSON on stdout** — a real inter-process wire — defined as `total=False` TypedDicts (`think/providers/shared.py:31-142`). The Python producer has no validator; R5b's native producer validates against `core/fixtures/cogitate_wire_contract.json`.

🆕 ⛔ **CORRECTED 2026-08-10 — "no schema, no fixture and no validator anywhere in the tree" was FALSE, and it was quoted into two wave scopes before anyone checked it.** `core/fixtures/callosum_registry.json` is a **committed fixture read by a Rust crate** (`solstone-core-callosum/src/registry.rs`), and its `cortex` entry enumerates **17 kinds**: `request · start · thinking · tool_start · tool_end · finish · error · talent_updated · info · status · cancel · dry_run · progress · text_delta · tool_budget_exhausted · warning · budget_escalation`. R5b adds `solstone-core-cogitate-wire`, its per-kind schema fixture, and `validate_event` for the native producer; the Python producer still lacks runtime validation. ⛔ **A rebuild must derive from that fixture or pin against it, failing in both directions** — publishing a second enumeration beside it creates two disagreeing lists of one vocabulary, which is the class this document exists to stop.

⚠ **And the enumeration is wider than the TypedDicts.** Of the 17, only 8 have a TypedDict and only **5** carry `Required` markers (`start`, `finish`, `talent_updated`, `thinking`, `text_delta`) — so `tool_start`, `tool_end` and **`error`** are all-optional, including `error`, whose absent `terminal` the consumer defaults to **true**. 📌 A schema "derived from the type markers" would therefore be weakest exactly where the risk is highest. ⚠ Three more asymmetries measured at the same time: **`talent_updated` has a TypedDict and no producer on this wire at all**; **`dry_run` is producer-side and cogitate-reachable** (`talents.py:2042`, emitted before the cogitate branch, so `--dry-run` puts it on this exact stdout wire) yet is untyped; and **`progress` is producer-side but generate-only**. **The shape most likely to be lost silently.** 📌 For contrast, the sibling vocabulary on the same bus *is* runtime-validated: `convey/chat_stream.py:137-138` raises on an unknown kind and `:375-386` enforces per-kind required fields. The native producer now validates its mapped subset; Python remains unvalidated.
...
⚠ **The TypedDicts are `total=False` but five of the eight carry `Required[]` markers** — `StartEvent`, `FinishEvent`, `TalentUpdatedEvent`, `ThinkingEvent`, `TextDeltaEvent`. That is a real static contract, simply never checked at runtime by the Python producer. ⚠ And the `Event` union **under-covers the wire**: `warning` (`providers/cli.py:425`), `tool_budget_exhausted` (`providers/cli.py:565`, `providers/openhands.py:432-448`) and `info` are emitted with no TypedDict at all.
...
| `S:web:journal` | `P-journal` | ⛔ 100% of `P-web` is `localhost:5015` or an authorized linked device. 🔵 **The localhost half is BUILT natively but is NOT yet what runs (2026-08-10).** `solstone-core convey` binds the loopback listeners and serves the shell, the app registry, the session gate and the speakers surface with no Python in the request path — proven against two frozen corpora and in a browser. ⚠ **But nothing starts it yet:** `journal convey` still resolves to `solstone/convey/cli.py` and Flask, from **both** supervisors (`think/supervisor.py` and `solstone-core/src/supervisor/runtime.rs`), so **Flask serves `:5015` today and all 21 apps are live.** The cutover is an `execv` from the existing Python entry point, and until it lands the native surface is a binary that works and never runs. 📌 **Building the server is not wiring it** — this is the same unreachable-crate shape the lane was chartered against, and it was nearly recorded here as done. ⚠ **At the cut the two halves diverge, and not symmetrically:** the linked-device half reaches convey through `convey/secure_listener/`, which terminates TLS in Python and **dispatches inline into the Flask WSGI app** — so when the Rust process takes the port, that door dispatches into nothing and **linked devices go dark**. ⛔ Authorized breakage, not a defect, and ⛔ not `P-web`'s to repair: the listener is `P-device-link`'s authorization boundary and Python by ruling. 📌 Recorded because a boundary that has quietly stopped working, with its code still on disk reading as live, is how a security surface rots |
