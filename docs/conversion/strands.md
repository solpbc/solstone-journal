...
🔴 **Under rule 1 only one name-variant matcher may own the contract, and the contract belongs to `P-entity`** — the identity-bearing store and the one-to-many end that every matcher's consumers resolve against. ⛔ **Consolidating matchers is not in flight and is not required of any current lane** — this is recorded so the class stops being representable, not as work to start. ⚠ A wrong identity decision here re-partitions an owner's people silently.

⏸ **UPDATED 2026-09-03: the three matchers this paragraph used to enumerate are down to one.** It named `think.entities.matching`, `apps/speakers/edges.py` and `entity_name_matcher.rs`. The first two went with the Python tree and no longer exist (`git ls-tree -r --name-only origin/main` finds zero `solstone/**.py`); the third is now `core/crates/solstone-core-entity-matching/src/matcher.rs`, whose `find_matching_entity` / `find_matching_entity_detailed` is the single tiered name matcher in the tree. ⚠ **Two siblings in that same crate are not equivalent to it, and the difference is load-bearing.** `build_name_resolution_map` (`batch.rs`) delegates straight to `find_matching_entity`, so it inherits the tier order and the ambiguity refusal. `is_name_variant_match` (`batch.rs`) does **not** — it recomposes the cascade's own primitives (`normalize_resolution_query`, `token_subset_match`, `prefix_token_match`) with no tier ordering, no exact/slug/email tiers, and **no ambiguity refusal**, so it can call two names variants where the tiered matcher would decline to decide. Its callers are `speaker-resolve/src/name_variant_scan.rs` and `talent-runtime/src/entities/review.rs`. ✅ The `P-entity` contract routing above is unchanged; only the enumeration was stale.
...
- ⚠ **Several consumers reach the formatters by direct import, bypassing the registry entirely** — the transcripts route, the activities route, and the text-projection wrappers. One of those cannot go through the registry by construction: it formats an in-memory record that has no file at all. ⛔ **A path-keyed registry cannot express that**, so the boundary needs a by-shape entry point that does not require a path.
...
| | | |
|---|---|---|
| **Bundled** | the three speaker/VAD graphs — `wespeaker-resnet34-256` · `pyannote-segmentation-3.0` · `silero_vad_v6` — and RF-DETR object-detection payloads | every install needs these assets; the speaker/VAD graphs are digest-pinned in `solstone-core-transcribe/src/model_assets.rs`, while RF-DETR is digest-pinned in `solstone-core-local/src/install/rfdetr_install.rs`, staged at `lib/solstone_journal_models/assets/rfdetr/`, and extracted and verified into the provider cache locally |
| **Fetched** | the large per-platform runtimes — `llama-server`, the Core ML transcription models, ced, the Vulkan probe's targets | `llama-server`, the Core ML transcription models, and the Vulkan probe's targets are gated on host capability or on an owner choosing a provider, so not every install needs those artifacts; ced is required parity that happens to be fetched; the mirror, catalog and deletion-guard machinery in `P-system-models` exists for exactly this class |
