...
⚠ The talent event vocabulary is **NDJSON on stdout** — a real inter-process wire — defined as `total=False` TypedDicts (`think/providers/shared.py:31-142`) with **no validator on the producing side**.

🆕 ⛔ **CORRECTED 2026-08-10 — "no schema, no fixture and no validator anywhere in the tree" was FALSE, and it was quoted into two wave scopes before anyone checked it.** `core/fixtures/callosum_registry.json` is a **committed fixture read by a Rust crate** (`solstone-core-callosum/src/registry.rs`), and its `cortex` entry enumerates **16 kinds**: `request · start · thinking · tool_start · tool_end · finish · error · talent_updated · info · status · cancel · dry_run · progress · text_delta · tool_budget_exhausted · warning`. ⚠ **So the vocabulary IS enumerated somewhere durable** — what it lacks is a producer-side validator and per-kind required fields. ⛔ **A rebuild must derive from that fixture or pin against it, failing in both directions** — publishing a second enumeration beside it creates two disagreeing lists of one vocabulary, which is the class this document exists to stop.

⚠ **And the enumeration is wider than the TypedDicts.** Of the 16, only 8 have a TypedDict and only **5** carry `Required` markers (`start`, `finish`, `talent_updated`, `thinking`, `text_delta`) — so `tool_start`, `tool_end` and **`error`** are all-optional, including `error`, whose absent `terminal` the consumer defaults to **true**. 📌 A schema "derived from the type markers" would therefore be weakest exactly where the risk is highest. ⚠ Three more asymmetries measured at the same time: **`talent_updated` has a TypedDict and no producer on this wire at all**; **`dry_run` is producer-side and cogitate-reachable** (`talents.py:2042`, emitted before the cogitate branch, so `--dry-run` puts it on this exact stdout wire) yet is untyped; and **`progress` is producer-side but generate-only**. **The shape most likely to be lost silently.** 📌 For contrast, the sibling vocabulary on the same bus *is* runtime-validated: `convey/chat_stream.py:137-138` raises on an unknown kind and `:375-386` enforces per-kind required fields. This one has the strictest static types in the tree and the weakest enforcement.
