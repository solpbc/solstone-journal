...
check-rust-journal-mcp-endpoint:
	@$(REQUIRE_CARGO)
	cargo test --manifest-path $(RUST_MANIFEST) -p solstone-core --features journal-mcp-endpoint --lib --bins --locked --offline --no-fail-fast -- --test-threads=1
...
# Final native Windows rail. This intentionally requires a clean commit and a
...
check-rust-ios:
	...
	if [ "$$(uname -s)" != "Darwin" ]; then \
		...
		cargo check --manifest-path $(RUST_MANIFEST) --workspace --exclude solstone-core --exclude solstone-core-journal-cli --exclude solstone-core-indexer-store --exclude solstone-core-indexer-query --exclude solstone-core-entity --exclude solstone-core-facets --exclude solstone-core-sol-link --exclude solstone-core-push --exclude solstone-core-spp-attest --exclude solstone-core-spp-ratls --exclude solstone-core-generate-wire --exclude solstone-core-transcribe --exclude solstone-core-convey-http --exclude solstone-core-convey-shell --exclude solstone-core-clients-web --exclude solstone-core-settings-web --exclude solstone-core-facets-web --exclude solstone-core-serving --exclude solstone-core-segment --exclude solstone-core-ingest --exclude solstone-core-entities --exclude solstone-core-speakers-analyze --exclude solstone-core-speakers-onnx --exclude solstone-core-describe --exclude solstone-core-observe-audio --exclude solstone-core-body-rebuild --exclude solstone-core-vad-analyze --exclude solstone-core-convey-body --exclude solstone-core-import-host --lib --target $(IOS_TARGET) --locked; \
	fi
