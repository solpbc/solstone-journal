...
- importing a file you already imported now says so. when a name clarification fails, it now distinguishes a busy journal, a missing suggestion or match, and a choice it could not apply.
...
- journal sources no longer use an API key. `journal importer journal-source create` doesn't give you one, and a key from a source you made earlier stops working and is removed from that source. sending into a source you made now works only from the computer your journal runs on. paired devices are unchanged and keep reaching your journal through their pairing.
...
- with a local model installed, opening home or thinking could wait while your journal reread the model's files. those pages now check setup quickly; starting the model still checks its files before it runs.
...
- when `journal backup run` saves a partial backup, it now says how many files it couldn't read and gives the first reason. until now it only said "some files couldn't be read".
...
- journal setup now sets a twenty-second stop limit on linux.
...
- when a connected device asks for too many things at once, `journal doctor` now identifies the connection and explains that it stayed up.
